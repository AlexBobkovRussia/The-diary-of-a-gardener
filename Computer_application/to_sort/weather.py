...
# Image
new = ImageTk.PhotoImage(Image.open('../Front/images/logo.png'))
...
if int((dt.strftime('%I'))) >= 8 & int((dt.strftime('%I'))) <= 5:
    img = ImageTk.PhotoImage(Image.open('../Front/images/moon.png'))
    panel = Label(root, image=img)
    panel.place(x=210, y=200)
else:
    img = ImageTk.PhotoImage(Image.open('../Front/images/sun.png'))
    panel = Label(root, image=img)
    panel.place(x=210, y=200)
...
def city_name():
    ...
    api_request = requests.get("https://api.openweathermap.org/data/2.5/weather?q="
                               + city_entry.get() + "&units=metric&appid=" + '95da50794a93c68df138e27bb27f4495')
